...
namespace StadiEm
{
    class StadiEmContext : ApplicationContext
    {
        ...
        ToolStripMenuItem emuDS4Item, emuXboxItem;
        ...
        public StadiEmContext()
        {
            ...
            emuDS4Item = new ToolStripMenuItem("Emulate Dual Shock 4 Controller", null, EmulateDualShock4Controller);
            emuDS4Item.Checked = false;

            emuXboxItem = new ToolStripMenuItem("Emulate Xbox Controller", null, EmulateXboxController);
            emuXboxItem.Checked = true;

            tray.ContextMenuStrip.Items.Add(emuXboxItem);
            tray.ContextMenuStrip.Items.Add(emuDS4Item);
            
            tray.ContextMenuStrip.Items.Add(new ToolStripMenuItem("Quit", null, Quit));
            tray.Text = "Starting...waiting for device";
        ...
        void Monitor()
        {
            while (runMonitor)
            {
                if (!emuXboxItem.Checked) goto IOErrorSkip;
        ...
        void EmulateXboxController(object sender, EventArgs e)
        {
            emuXboxItem.Checked = !emuXboxItem.Checked;

            if (!emuXboxItem.Checked)
            {
                //t_monitor.Join();
        ...
        void EmulateDualShock4Controller(object sender, EventArgs e)
        {
